  ...
  module JSMethod
    ...
    def define_js_method(method_name, code, options = {})
      ...
      method_proc = Proc.new do |*arguments|
        puts "js_method proc, arguments:#{arguments.inspect}"
        parsed_arguments = []
    ...
    def call_js_method(definition_receiver_js, name_js, arguments_js)
      ...
      capture_exception do
        return_js = js_class_interface.call(key_js, definition_receiver_js, name_js, arguments_js)
      end
    end
    ...
    def capture_exception(&block)
      return_js = yield
      # avoid infinite loop with an unsafe call (no exception capturing)
      #if js_class_interface.call(JSInterface.string("is_error"), return_js).to_boolean
      #  raise_js_exception(return_js.get(JSInterface.string("error_js")))
      #end
      return_js
    end


    def raise_js_exception(error_js)
      error_js = new(error_js)
      error_rb = nil
      if error_js.instanceof(get("BRubyBridge.RbError"))
        error_rb = error_js["js_value"].to_rb_object
      else
    ...
    def interface_array(array_rb)
      puts "interface_array array_rb:#{array_rb.inspect}"
